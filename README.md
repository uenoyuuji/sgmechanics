...
stamina = Sgmechanics::Stamina.new(
  seconds_per_stamina, # 1回復するのに必要な秒数 (マスターデータなどから指定)
  max_stamina,         # スタミナの最大値 (マスターデータなどから指定)
  completion_time,     # 回復が完了する時刻 (Unix タイムスタンプ)
  overflowed_stamina,  # 最大値を超えた分 (通常は 0)
  Time.now.to_i        # 計算時刻 (Unix タイムスタンプ)
)
...
# 復元
stamina = Sgmechanics::Stamina.new(
  seconds_per_stamina, max_stamina,
...
lc.date_for(now)                        # => Date (ゲーム内の今日)
lc.previous_reset(now)                      # => Time (直近のリセット時刻)
...
lc.date_for(Time.new(2024, 1, 10, 4, 59, 59, "+09:00"))
# => #<Date: 2024-01-09>

# JST 2024-01-10 05:00:00 → ゲーム内は 1月10日
lc.date_for(Time.new(2024, 1, 10, 5, 0, 0, "+09:00"))
# => #<Date: 2024-01-10>
```
